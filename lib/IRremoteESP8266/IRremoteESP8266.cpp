...
void IRsend::sendCOOLIX(unsigned long data, int nbits) {
    ...
    for (int j = 0; j < COOLIX_NBYTES * 2; j++) {
        for (int i = nbits; i > nbits - 8; i--) {
            COOLIXmask = (unsigned long) 1
                    << (i - 1);  // Type cast necessary to perform correct for the one byte above 16bit
            if (data & COOLIXmask) {
...
void IRsend::sendGC(unsigned int buf[], int len) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        for (; j < len; j++) {
            int microseconds = buf[j] *
                               periodic_time; // Convert periodic units to microseconds. Minimum is 80 for actual GC units.
            if (j & 1) {
